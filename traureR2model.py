...
def main():
    ...
    for i in range(rang_minim, rang_maxim + 1):
        ...
        if  (regressorPar == 1):
            model = RandomForestRegressor(n_estimators=62, random_state=42, max_depth=20)
        elif(regressorPar == 2):
            model = LinearRegression()
        elif(regressorPar == 3):
            model = DecisionTreeRegressor(random_state=42, max_depth=20, min_samples_leaf=18)
        elif(regressorPar == 4):
            model = GradientBoostingRegressor(random_state=42, n_estimators=63,max_depth=5)
        elif(regressorPar == 5):
            model = SVR(gamma=1/200,epsilon=0.001,C=1200)
        elif(regressorPar == 6):
            model = KNeighborsRegressor(n_neighbors=5,weights='distance',metric='minkowski')
        elif(regressorPar == 7):
            model = MLPRegressor(random_state=42,hidden_layer_sizes=(i,i),solver='lbfgs', max_iter=5000,alpha=5)
        else:
            model = None
    ...
    #print("-" * 30)
        print(f"RESULTATS DE LA VALIDACIÓ (ANY 2025): weights='distance',metric='minkowski'")
        #print("-" * 30)
...
# --- 5. GENERACIÓ DE GRÀFIQUES COMPLETES (2019-2025) ---
    ...
    # Això crearà la línia ROJA contínua
        ...
        nom_arxiu_model = 'model_et0_SVR.joblib'
        #joblib.dump(model, nom_arxiu_model)
        joblib.dump(model, 'model_et0_svr.joblib')
    ...
    # Mostrem la importància de les variables
        ...
        #importancies = pd.Series(np.abs(model.coef_), index=cols_input).sort_values(ascending=False)
